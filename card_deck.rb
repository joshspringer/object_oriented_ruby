...
class Card
  def initialize(trivia_data)
    @question = trivia_data.keys[0]
    @answer = trivia_data.values[0]
  end
end
...
p trivia_data.values[0]
p trivia_data.keys[1]
p trivia_data.values[1]
p trivia_data.keys[2]
p trivia_data.values[2]
